...
turn a text label into a picture, which can then be treated like any other.  It can
...
\MP\ input is strictly 7-bit ASCII only, but you can use 8-bit characters, including
UTF-8, as the payload of a string.  As noted above, the PostScript font names used
by "infont" are defined to be a combination of encoding and font glyphs, so each
different font name implies a particular encoding.  All of these encodings are 8-bit
encodings, so the maximum number of characters available in each font is 256, and there is no
support for Unicode.\mpic{-81pt}{font-sampler-ugmr8r.pdf}  This means that although
you can put UTF-8 characters in a string, they will be printed as a mess, so you
need to stick to regular 8-bit characters or use the "char" mechanism.

If you want to use "infont" with Computer Modern, you might choose "cmr10" as used
by plain \TeX, however as soon as you set a label with more than one word, you will
find this font has an acute accent instead of a space in postion 32 To fix this use
the Latin Modern versions of the fonts, with a more standard encoding. For "cmr10"
use "texnansi-lmr10", for "cmss10" use "texnansi-lmss10" and so on.  

The "texnansi" encoding is almost the same as ISO Latin~1, which was the widely used
Windows standard before Unicode.  For the standard Adobe fonts like Helvetica and
Times New Roman it's best to stick to the "8r" encoding, which is even closer to ISO
Latin~1.  
...
\end{code}
Armed with these measurements you can align your text neatly so that it is
vertically centered on the lower case letters regardless of any ascenders or
descenders.
...
explain how label macros work - only primitive is draw! 

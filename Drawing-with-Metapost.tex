...
\end{code}
\vss}}
$$\includegraphics[width=\textwidth]{brownian-walks-red-blue}$$
...
Note the first line of the \kw{hide} block adds a normal random value to $y$.
...
example:
\begin{code}
y := y if p > uniformdeviate 1: + else: - fi 1; 
\end{code}
will set the delta positive with probability $p$ and negative with probability $1-p$.
...
these two tiny rectangles, which is why they do not always refer to exactly the same
point.}}

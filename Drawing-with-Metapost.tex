...
\newcommand\mpinline[1]{{\mpstyle\lstinline!#1!}}
\begin{document}
\pagestyle{empty}
\raggedbottom
...
\end{code}
    ...
    \item Variable types: 
        ...
        —
        \mpinline{color} "(r,g,b)",
        \mpinline{color} "(c,m,y,k)"
        \mpinline{transform} "(x,y,xx,xy,yx,yy)"
        ...
        If you don’t declare a variable, it’s assumed that it’s a \mpinline{numeric}.
        When you do declare a variable --- \mpinline{numeric} or otherwise — any value that it
...
any resolution less that 150,000 dots per square inch.  If you were designing fonts
...
\newpage
\section{Pairs, colours, and transformations}

\MP\ generalizes its concept of number to include ordered tuples of numbers which are used for
a variety of purposes.  The primary use of pairs is for Cartesian coordinates, but
they can also be used as complex numbers, since 
as far as \MP\ is concerned it's just a pair of numeric variables.

Triples and quads are used for colours (RGB colours and CMYK colours); usually the
values of each element vary from 0 to 1, so that $\id{red} = (1,0,0)$ for example, 
but \MP\ does not force you to stick to this range.  The elements can take any value
that is allowed for a \kw{numeric}.  It is therefore possible
to use triples for three dimensional coordinates, but there are no
built-in facilties in plain \MP\ that support this, so at best it is rather
cumbersome.  

Finally there are sextuples used for transforms.  A transform is how \MP\ represents
an affine transformation such as "rotated 45 shifted (10,20)".  The gory details of
these are in the \MF book.
...
\id{A},\id{B};$' before you assign values to them.  Once you have declared them, you
can equate to an appropriate tuple using $=$ as normal.
\begin{code}
    pair A,B; A = B = (3,4);
    color R;  R = (1,2,3);
...
\end{code}
Triples are type \kw{color}, quads are type
\kw{cmykcolor}, sextuples are type \kw{transform}.  You can't have tuples of any
other length, not even as constants.
...
counter-clockwise from the positive $x$-axis, you can write `\mpinline{r dir
theta}'.  For example `\mpinline{2 dir 30}' provides another way to get the
point "(sqrt(3),1)".   
And if that's not compact
enough for you could define a suitable shorthand using a character that's otherwise
unused.  For example `\mpinline{def ^=dir enddef;}', which would let you write
`\mpinline{2^30}'.
...
works well when applied to a path
or an image.
...
\MP\ pair variables work rather well as complex numbers, as you might expect in a
language designed by a mathematician.  To represent $3+4i$ you can write "(3,4)".
To get the modulus you write "abs (3,4)" (which gives $5$ in this case), and to get
the argument you write "angle (3,4)" (which gives $53.1301$).  Note that "angle"
returns the argument in degrees rather than radians, and that the result is normalized so
that $-180 < \kw{angle} (x,y) \le 180$.

The standard notation for points supports this.  You can write "z0=(3,4);" and then
...
to do the same thing. So after `\mpinline{pair w; w=(3,4);}' you can get the real part with
"xpart w" and the imaginary part with "ypart w".

There is also a neat way to write complex numbers using a polar notation.  For
$re^{i\theta}$ you can write `\mpinline{r dir theta}' where "r" is the modulus and
"theta" is the argument in degrees. So another way to get the point "(3,4)" is to
write `\mpinline{5 dir 53.1301}'. For points on the unit circle you can omit the
"r", so `\mpinline{dir 45}' gives you the point "(0.7071,0.7071)" and so on. 
And if that's not compact enough for you could write `\mpinline{def ^=dir enddef;}'
which would let you write `\mpinline{5^53.1301}'.

The predfined constants \mpinline{up}, 
\mpinline{down}, 
\mpinline{left}, and  
\mpinline{right} also provide points on the unit circle corresponding to $i$, $-i$,
$-1$, and $+1$ respectively.  It's tempting to define \mpinline{pair i; i=(0,1);}, so that
...
\mpinline{(3,4) zscaled 4}.  To get that effect with "zscaled" you would have to write 
\mpinline{(3,4) zscaled (4,0)}, but this is the same as 
\mpinline{(3,4) scaled 4}, which is usually simpler to write.  If your pair is
stored as a variable you can write (for example) \mpinline{4 z0} to get the same
effect.  

Or \mpinline{1/4 z0} or \mpinline{z0/4} for scalar division.

There are no other complex operators available, but it is not hard to implement the
usual operations when they are required.

For complex division you need a function to inverse 
divisor.  
Inversion
Complex sqrt
...
\noindent\vadjust{\moveright5.5in\vbox to 0pt{\hsize4in\noindent
...
\moveright5.5in\vbox to 0pt{\hsize4in\noindent
        ...
        \centerline{Lee Sallow's theorem of median triangles}
}
\noindent
The best way to bisect a line, depends on how you have defined it.
If you have two points $a$ and $b$, then the simplest way to find 
the point that bisects them is to write "1/2[a,b]".  This mediation 
mechanism is entirely general, so you can write \mpinline{1/3[a,b]}, "1/4[a,b]", and
so on to define other points that are part of the way from $a$ to $b$.
The point "0[a,b]" is equal to point $a$, point "1[a,b]" is equal to $b$;
but the number before the left bracket does not have to 
be confined to the range $(0,1)$.  If you write "3/2[a,b]" you will get a point on
the extension of the line from $a$ to $b$ beyond $b$.  To get a point going the
...
the order of the points: "(3/2)[b,a]".

If you have a path to bisect then you can use the ``$\kw{point} t \kw{of} p$''
notation to do mediation along the path.  For a simple straight path $p$ of length 1
then "point 1/2 of p" will give you the midpoint.  More generally, 
"point 1/2 length p of p" will give you the midpoint of a path of any length. 
This works fine for simple paths, in which \MP's time moves evenly along the path, 
but for more complicated paths you have to use this rather cumbersome notation:
\begin{code}
...
triangle.  You could find it "intersectionpoint" or with
a construction using "whatever" on any two medians, but since we know that the centroid
...
calls to "intersectionpoint".  If you wanted to eliminate this you could use the 
...
\textit{This section describes labels and annotations in what can be called the traditional
\MP\ environment, where your figures are compiled with "mpost".  The following
section describes labels and annotations in the new-fangled world of "lualatex" and
...
will see this as Ã©.  But you can write a fairly simple routine to decode the
...
\begin{code} 
for i=0 upto 4: 
  label(btex "$p_" & decimal i & "$" etex, (10i,0)); % this won't work
endfor 
\end{code} 

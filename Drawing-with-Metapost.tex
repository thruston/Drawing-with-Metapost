...
The circumcircle of a triangle is the circle through the three corners, so you can
use the \mpl{circle_through} macro from §\ref{sec:circles} that uses an idea from
§\ref{sec:parallel}.
\begin{code}
vardef circle_through(expr A, B, C) =
    save m; pair m;
    m = whatever * (A-B) rotated 90 shifted 1/2 [A,B]
      = whatever * (B-C) rotated 90 shifted 1/2 [B,C];
    fullcircle scaled 2 abs (A-m) shifted m
enddef;
\end{code}
Note that as the diagram on the right shows,\mxpic{-1in}{4.8in}{circumcircle}
...
\begin{code}
    m = whatever * (A-B) rotated 90 shifted 1/2 [A,B]
      = whatever * (B-C) rotated 90 shifted 1/2 [B,C]
      = whatever * (C-A) rotated 90 shifted 1/2 [C,A];
\end{code}
but it does not add any more information to the system of equations, and
\MP\ will sometimes give you an “inconsistent equation” error.
...
enddef;
\end{code}
The triple marks on the segments of $AC$ were created with \mpl{mark_equal(C,A,3)}.
...
transformation.\mpic{0pt}{func-exponential}
\begin{smallcode}
numeric u; u = 42;
path xx, yy;
xx = (-3u, 0) -- (5u, 0);
...
\vskip -84pt\noindent
With a small step size $s$ and a base circle \id{base}, this suggests:
\begin{smallcode}
...
\begin{itemize}
    \item Here $A$ has been moved to the left, but\dots
    \item Each $P$ on the curve is still $A$ reflected in the tangent
        at each $Q$.
    \item The “hole” gets larger as $A$ moves away from the \id{base} circle.
    \item When $A$ touches the \id{base} the hole disappears.
    \item The curve then becomes the cardioid, as before.
\end{itemize}

\noindent
The red limaçon path was generated like this:
\begin{smallcode}

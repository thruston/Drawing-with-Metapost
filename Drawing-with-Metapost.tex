...
that is allowed for a \kw{numeric}.  It is therefore possible to use triples for
three dimensional coordinates, but it is not very easy or elegant, as
there are no built-in facilties (such as "draw3d") in plain \MP\
to support it.
...
Except for \kw{transform} variables, you can't have tuples of any other length, not even as constants.

A transform is how \MP\ represents
an affine transformation such as "rotated 45 shifted (10,20)".  
They are represented as sextuples, but if you try to write:
\begin{code}
...
\MP\ implements colours as tuples of three or four numeric values.
Three-tuples (which are type \mpl{color}) represent RGB colours; four-tuples
(which are type \mpl{cmykcolor} represent CMYK colours.  The numeric values 
can take any legal numeric value, but \MP\ only considers the range 0 to 1.  Values
less than zero are treated as zero, values greater than 1 are treated as 1.  So to
encode a color, such as British Racing Green with RGB code "(1, 66, 37)" you should
write:
\begin{code}
    color brg;
...
calculated value.  \vadjust{\moveright 384pt\vbox to 0pt{\kern-12pt
\begin{code}
N = 5; n = 0;
for y=1 upto N:
  for x=1 upto N:
...
\vbox to 0pt{\centerline{\includegraphics{scatter-color1}}\vss}

\newpage
\subsubsection{CMYK colours}
...
While we are on the topic of colour models, here is a short digression on the use of grey
scale (or gray scale if you prefer the Webster spellings) in \MP.


\newpage
\subsubsection{Colorbrewer palettes}

The well-known Colorbrewer website ("http://colorbrewer2.org") provides a wide range
of useful colour palettes that are suitable for a wide range of applications.

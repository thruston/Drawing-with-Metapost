...
egg-shaped path from a series of circular arcs.  

\subsection{Euclidean egg}

The first is made of four circular arcs,\mwpic{0pt}{eggs-moss}
defined here as parts of circles $a$, $b$, $c$, and $d$.  But note
that you don't actually need $d$ to make the egg; it is enough simply
to join the arcs of $b$ and $c$ with the $\to$ connector.
\begin{code}[xleftmargin=0pt]
color eggshell; eggshell = 3/4[1/4[red, green], white];
path a, b, c, d, egg; numeric r; r = 72;
...
egg = subpath (4, 8) of a & b .. c & cycle;
fill egg withpen pencircle scaled 2 withcolor eggshell;

picture P; P = currentpicture;
drawoptions(withpen pencircle scaled 1/4 withcolor 1/2);
draw a;
draw b -- point 4 of a -- cycle;
draw c -- point 0 of a -- cycle;
draw d;
...
\end{code}
Note that you can use $\to$ to create reasonably large 
circular arcs.
...
A slightly different approach using a $\sqrt3$-2-$\sqrt7$
...
numeric a, b, c;
a = abs(z3 - z1); b = a - abs(z4 - z6); c = a - abs(z4 - z5);

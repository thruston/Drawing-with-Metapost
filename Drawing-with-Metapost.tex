...
\def\mptype#1{\ensuremath{\langle\hbox{#1}\rangle}\thinspace}
%
...
matches "*.mp", my editor profile sets the filetype to "mp" (which picks up the
...
using the \texttt{standlone} class, which automatically adjusts the page size to
...
every time you compile the document, which might slow you down.
...
\begin{itemize}
    ...
    \item "fullcircle" which you can use to draw any circle or ellipse with
        appropriate use of "xscaled" and "yscaled".  It is defined so that it is centered
...
\end{verbatim}
and follow the installtion instructions provided there.
%--------------------------------------------
\newpage
...
"plain.mp") using two \MP\ primative commands:
\begin{code}
...
1\unit{bp} centered on the point $(0, 0)$.  To use it, you can scale it and shift it.
...
\textit{diameter}, not unit \textit{radius}.  To draw a circle centered at point $A$
...
the center of $C$; draw a circle through $p$, centered on this midpoint; the tangent
...
centered at the same point as $A$ and scaled so that its radius is $R-r$.  If we
...
\bigskip
\noindent\hbox to \textwidth{\includegraphics[scale=0.95]{axis1}\hss}
\vadjust{\moveright5.5in\vbox to 0pt{\hsize 4in\vss\noindent
...
$I_{31}$, and $I_{23}$).  These points have a pleasing colinearity.
...
the radius of the circles centered at the \mpl{pair} variables $C_1$ and $C_2$, 
then we have:
...
the given font.
$$\mptype{string} "infont" \mptype{string} \longrightarrow \mptype{picture}$$
With the option enabled,
the right hand \mptype{string} argument (which names the font) is completely ignored,
and the left hand \mptype{string} argument (the text to show) is passed to the "TEX()"
macro. The result is still a \mptype{picture} of course, but instead
...
Note that even with the option enabled, \MP\ will not let you pass a \mptype{string}
...
syntax scheme as a special pair of quotation marks that produces a \mptype{picture}.
...
\mptype{picture}, and \mpl{&} only works with strings or paths.


\newpage
\subsection{Transforming text pictures}

Whether you call the "luamplib" \mpl{TEX()} macro explicity, or implicitly using
"infont", "btex" \dots\ "etex", or the "mplibtextextlabel" option, the result is a
\mptype{picture} that can be processed like any other.  Since \MP\ does not care if
the text picture you have created came from traditional "infont" or new-fangled
...
transformation to it, such as scaling or a rotation.  This section add to that
...
% done - HSV colours, brewer colours, rainbows, 50\% white etc

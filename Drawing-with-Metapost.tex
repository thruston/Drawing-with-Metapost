...
\vadjust{\moveright 3.4in\vbox to 0pt{\includegraphics[scale=0.8]{clocks1}\vss}}
...
\centerline{\includegraphics{complexities.pdf}}
The interesting part drawn as follows:
\begin{smallcode}
...
label.ulft("\texttt{zinverse()}", 42 dir 120);
\end{smallcode}
\vss}}
...
\moveright 384pt\vbox to 0pt{\vskip36pt\raggedright\hsize4in\noindent
...
label.top("Remain" infont "phvr8r", (s, s));
label.top("Leave" infont "phvr8r", (10s, s));
\end{smallcode}

\bigskip
$$\includegraphics{brexit-palette1.pdf}$$
...
colours show how we voted.  The data is from publicly-available UK
...
were originally written for maps, but they have a wide range of applications. 
...
another for RGB; an example of usage is shown over there~$\longrightarrow$

$$\includegraphics[width=\textwidth]{brexit-map.pdf}$$


\smallskip\noindent
If you need to install the package by hand you can get it from CTAN at:
\begin{verbatim}
...
$$\hbox to \textwidth{\includegraphics{random-simple2}\hss}$$
...
\includegraphics{intersectme0}&\includegraphics{intersectme2}&\includegraphics{intersectme3}\cr
}}$$
...
$$\vbox{\halign{$\vcenter{\hbox{#}}$&&\quad$\to\vcenter{\hbox{#}}$\cr
\includegraphics{intersectme4}&\includegraphics{intersectme5}&\includegraphics{intersectme6}&\quad$\cdots$\cr
}}$$
...
\mpic{-12pt}{circles1.pdf}
The path is defined (in
"plain.mp") using two \MP\ primitive commands:
\begin{code}
...
points are where this second circle intersects $C$.\mpic{-120pt}{tangents3.pdf}
...
circles, using an approach like this: \mwpic{-12pt}{tangents5.pdf}
\begin{code}
path A, B;
A = fullcircle scaled 144;
B = fullcircle scaled 60 shifted (240, 80);
...
other side:
$$\includegraphics[width=\textwidth]{tangents55.pdf}$$
...
"\hfil" commands. \mpic{-160pt}{labels2}
...
new-fangled facilities you need to create your \MP\ diagrams inside a \TeX-wrapper
...
\clearpage
\subsection{The magic of the \texttt{textextlabel} option}
...
\subsection{Using Unicode and matching style with OTF fonts}
...
the contents of these variables are a bit more complicated.  The system keep track
of the picture it is working on in a variable called \mpl{currentpicture}, which can
be copied to your own variables, or manipulated in various useful ways.  In \MF\ the
contents of the variable is a pattern of pixels for a font, in \MP\ the contents are
vector graphics commands.  This section reviews some of the things you can do with a
\<picture> variable (including putting it in a frame $\longrightarrow$).
\mpic{-96pt}{youth}
...
magic of macro grouping to make it easier.
\begin{code}
vardef image(text t) =
  save currentpicture;
...
\end{code}
and \MP\ will add a copies of $P$ with the reference points shifted to $(20,0)$,
...
It is useful as an alternative to \mpl{buildcycle}, but most commonly used
for trimming a repeating pattern to a particular shape.  

The usual approach is to define a particular shape, $s$, then draw your pattern over
a large area that covers the shape, and finally call \mpl{clip currentpicture to s}
to trim the pattern to the shape.  This is best done after you define all your
shapes, but before you draw any others or add any labels.  The example on the right
shows this approach in action.  

But you can apply \mpl{clip} to any picture, so you might prefer to capture your
...
\vfill\noindent
Why would you ever want to use \mpl{setbounds}?  Mainly to help with aligning type set labels,
...
corners.
$$\includegraphics[width=5in]{corners}$$
You will notice this if you use the technique given on p.29 of the \MP\ manual to draw a
label on a coloured (or erased) background;  if you have rotated the label, the
\mpl{bbox} may be larger than you want. The solution is to define your label
untransformed and then apply the transformation twice: first when you fill the bounding box 
and then again when you draw the label, for example:
\begin{code}
...
then use \mpl{image} to draw each part separately.  The advantage of this is that do
you not have to worry about where the \mpl{origin} is, which often makes a drawing
simpler (for example because you can use \mpl{rotated} rather than
\mpl{rotatedaround}).  Once you have created all the parts you can then add them to
the final image using \mpl{draw} or \mpl{label} as shown on the right
$\longrightarrow$

\moveright 396pt\vbox to 0pt{\hsize 4in \vss
\noindent The comparison picture above was drawn using this general sub-picture
...
draw P1 shifted 100 left; draw P2 shifted 100 right;
\end{code}
Sometimes it is more convenient to use \mpl{label} to place the pictures, taking
advantage of the automatic alignment provided.  

Note also that the blank parts of each
picture are really transparent, unless you have explicitly filled them with
\mpl{white} colour, so you can overlap them when appropriate.
}
...
will produce a label neatly centred at the at the bottom.  If it is too close
you can either set a larger \id{bboxmargin} or use
\begin{code}
label.bot("This picture needs a label at the bottom", 
...
\noindent
The path returned by \mpl{bbox} has four points starting at
the lower left and proceding clockwise like a \mpl{unitsquare}. So 
\mpl{point 1/2 of bbox currentpicture} is half way between lower left 
and lower right,
while \mpl{point 5/2 of bbox currentpicture} is half-way from
...
centered on the origin and scaled to \id{bboxmargin}.

\moveright5.5in\vbox to 0pt{\hsize 4in \vss
\noindent
Here is an example.
$$\includegraphics[width=4in]{double-angle}$$
The labels at the bottom where added like this:
\begin{smallcode}
...
Adding a background after finishing
Adding a border
...
Whether you call the "luamplib" \mpl{TEX()} macro explicitly or implicitly using
"infont", "btex" \dots\ "etex", or the "mplibtextextlabel" option, the result is a
\<picture> that can be processed like any other.  Since \MP\ does not care if
the text picture you have created came from traditional "infont" or newfangled
\mpl{TEX()}, the discussion in §\ref{infontbbox} applies just the same.  But apart
from adjusting the bounds to work well with \mpl{label} you can also apply a
transformation to it, such as scaling or a rotation.  This section adds to that
discussion some details of what happens to the bounding box of the text when you do
any of these things.  
...
\newpage
\subsection{Cycloid through a point}
...
needs are simpler, then plain \MP\ is more than capable.  This section presents some ideas and

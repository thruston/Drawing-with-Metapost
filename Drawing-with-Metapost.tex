...
\textsc{Another aperiodic tiling} is the so-called pinwheel tiling, based on
Conway's dissection of a $2\times1$ right angle triangle.\mwpic{-64pt}{pinwheel}
$$\includegraphics{conway.pdf}$$
Starting with a triangle of this shape, the tiling recursively divides into
five smaller copies of itself.  An extra level of apparent randomness is added in
this version by varying the change in level for some of the triangles.
\begin{smallcode}
input colorbrewer-rgb
vardef pinwheel(expr level, a, b, c, s) = 
    if level < 1:
        ...
        pinwheel(level - 1, e, a, c, Blues 9 2);
        pinwheel(level - 3, f, g, a, Blues 9 3);
        pinwheel(level - 2, f, g, e, Blues 9 4);
        pinwheel(level - 1, d, e, g, Blues 9 5);
        pinwheel(level - 2, d, b, g, Blues 9 6);
    fi
enddef;
beginfig(1);

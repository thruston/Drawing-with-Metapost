...
pairs: the point, the pre-control point, and the post-control point. Unless you
explicitly set them differently, \MP’s curve fitting will make these three points co-linear.  The idea is
that adjusts a path so that when it travels through a point, it is heading in
...
path p; p := origin {1,3} .. (192,0) .. {1,2} (384,0);
\end{smallcode}
it has three points – \id{origin}, $(192,0)$, $(384,0)$ – numbered 0, 1, and 2;
and you can extract
any of these points with \mpl{point t of p}.  It also has control points which
we have constrained at point 0 and 2 but left free at point 1.
You can extract the control points 
...
for t=0 upto length p:
  drawarrow precontrol t of p -- point t of p --  postcontrol t of p;
endfor
\end{smallcode}
you get a neat visualization of how the path is made.  Now, plain \MP\ defines
...
\begin{code}[xleftmargin=0pt]
pair o, t, t'; o = center C;
t = C intersectionpoint halfcircle zscaled (p-o) shifted 1/2[o,p];
t' = C intersectionpoint halfcircle zscaled (o-p) shifted 1/2[o,p];
\end{code}
...
to give equations for three non-colinear points, and \MP\ will work out 

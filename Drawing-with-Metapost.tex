...
\newpage
\subsection{Fitting dashed lines to a cyclic path}
...
paths exactly -- this is especially noticeable with cyclic paths, where you may end
...
\begin{itemize}
    \item `$\kw{uniformdeviate}\,n$' generates a random real number between $0$ and
        ...
        are all multiples of $1\over4096$, so $\kw{uniformdeviate} 8092$ (for
        example) generates even integers instead of random real numbers, then you
        should update your \TeX\ distribution.  This `feature' was an accident of
        the original way that the scaled arithmetic routines were
        adapted.
...
beginfig(2);
  ...
  draw_out(t,6); % the freehand triangle defined above
...
math italic layout --- essentially it has (nearly) all the Greek letters but none of the
arrows, nor the music notation.
...
makes it more convenient when you want to add a number
scale.\mpic{0pt}{numbered-axes}
...
for x=-1 upto 4:
    if x <> 0:
        draw (down--up) shifted (x * u, 0);
        label("$" & decimal x & "$", (x * u, -8));
    fi
endfor
for y=-1 upto 3:
    if y <> 0:
        draw (left--right) shifted (0, y * v);
...
For this diagram, the sine wave path ("ss", shown in \textcolor{textblue}{blue})
...
drawn in red, and chopped off to fit the same width.
...
Each point on the parabola path is related to each point, and you can contruct an
...
\end{code}
The length of the semi-major axis, $a$, and the semi-minor axis, $b$, are given by
\begin{code}
numeric a, b;
...
\textsl{Mathographics}.  They all follow the same basic idea of contructing the
...
and showing the contruction are similar to the first example.
...
but this comes at the cost of more complexity and the loss of the 
points at exactly E, N, W, and S.\mwpic{0pt}{eggs-five-point}
\begin{smallcode}[xleftmargin=0pt] 
numeric a; a = 56;    
path r[], s[];
for i=1 upto 4:          % -2.5a so the origin is in centre 
    r[i] = fullcircle scaled 2a shifted (0, a * i - 5/2 a);  
endfor
r5 = r2 rotatedabout(point 2 of r2, 60);
r6 = r2 rotatedabout(point 2 of r2, -60);
z1 = subpath (4, 8) of r5 intersectionpoint 
         (point 6 of r6 -- 8[point 6 of r6, point 2/3 of r2]);
z2 = subpath (0, 4) of r4 intersectionpoint 
         (point 6 of r6 -- 8[point 6 of r6, point 2 of r3]);
z3 = whatever[z1, point 6 of r6] = whatever[point 2 of r3,point 10/3 of r1];
z4 = 1/2[point 10/3 of r1, point 2/3 of r1];
z5 = whatever [z3, point 2/3 of r1] = whatever [z4, point 10/3 of r3];

s1 = fullcircle scaled 2 length (z2 - point 6 of r6) 
                shifted point 6 of r6 cutbefore z1 cutafter z2;
s3 = fullcircle rotated angle (point 2/3 of r1 - z3) 
                scaled 2 length (z1 - z3) shifted z3 cutafter z1;
s5 = fullcircle rotated angle (z4 - z5) 
                scaled 2 (length (z1 - z3) - length (z3 - z5)) 
                shifted z5 cutafter point 0 of s3;
...
$$\includegraphics[width=0.9\textwidth]{eggs-perfect.pdf}$$
% eggs-pentagons.mp
% eggs-perfect.mp
% eggs-shaded.mp
% eggs-fried.mp
...
\end{mplibcode}
\end{smallcode}
Instead of defining six trianges, six squares, and a hexagon, you can just define

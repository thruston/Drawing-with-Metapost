...
\newpage
\subsection{Bounding boxes for text}

Whether you call the
"luamplib" \mpl{TEX()} macro explicity, or implicitly using "infont", "btex" \dots\
"etex", or the "mplibtextextlabel" option, the result is a \mptype{picture} that
processed like any other.  This means you can pass is to the \mpl{label} macro, or
just place it directly in your drawing with the \mpl{draw} command; it also means
you can apply a transformation to it, such as scaling or a rotation.  This section
attempts to show what happens to the bounding box of the label when you do any of
these things.

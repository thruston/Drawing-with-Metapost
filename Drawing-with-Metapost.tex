...
line means that the output will be written to files with an extension of ".eps"
(suggesting that they are Encapsulated PostScript).
...
\moveright5.4in\vbox to 0pt{\vss\hsize 4in\noindent
...
disadvantage is that you can only produce one PDF output file, so you need to have a
...
\moveright 384pt\vbox to 0pt{\vskip2.2in\raggedright\hsize4in\noindent
...
another for RGB.  If you are using an up-to-date \TeX\ distribution, you should find 
that they are already installed, but if not you just have to put the files into your local TeX tree so that
\MP\ will find them.  On my Mac this means putting the files into
"~/Library/texmf/metapost".

\vbox to 0pt{$$\includegraphics[width=\textwidth]{brexit-map.pdf}$$\vss}
...
circles $A$, $B$, or $C$, is rotated (that is that point 0 is at 3 o'clock).  But
...
(that plain \TeX\ uses for the \L\ character) instead of a space, and that you can still use PostScript
fonts like "phvr8r".  But also notice that the "TEX()" macro and the "btex" \dots\ 
"etex" construction have picked up the font set by the \LaTeX\ wrapper.  As you can
see they produce exactly the same output; "TEX()" is more useful because you can 
pass a primary string variable as an argument, which makes it easier to
construct dynamic labels.  "TEX()" also has the synonym "textext()" for compatibility
with ConTexT.  You can use either name, as you prefer.  
...
\subsection{Matching style with Unicode and OTF fonts}

matching fonts
using UTF8 and OTF fonts (and greek)

\subsection{Typographical minus signs for labels}

This is really easy now, because we can assemble a string
on the fly:\mpic{0pt}{simple-number-line}
\begin{code}
...
\end{code}
You will recall that the "&" binary operator concatenates two strings.

\subsection{Dynamic labels}

This is also really easy, for the same reason.  The code to label
points round a circle is now simple \textit{and} quick:\mpic{0pt}{simple-circle-labels}
\begin{code}

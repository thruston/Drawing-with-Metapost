...
The canonical way to find the intersection between two straight lines, is to use
the mediation syntax with \id{whatever} twice:
$$\includegraphics[width=0.9\textwidth]{whatever.pdf}$$
The generality of the mediation syntax means that this works even if the two line
segments don't intersect.  The intersection will be the point where the 
two (infinite) lines through the pairs of points meet.  If the two lines
are parallel, you'll get an `inconsistent equation' error.
If you want to capture the calculated values, then use 
undefined numeric variables instead of \id{whatever}:
\begin{code}
    z0 = alpha [z1, z2] = beta [z3, z4];
\end{code}
In this example you would find $\alpha=0.286$ and $\beta=0.5$.
If one of your straight lines is horizontal or vertical, then you only need one
...
\smallskip\noindent
For paths that are more complicated than straight lines, you 
need to use the \mpl{intersectiontimes} primitive or the \mpl{intersectionpoint}
macro.


\subsection{Parallel and orthogonal lines}

...
figures are compiled with "mpost".  The next section describes labels and
annotations in the newfangled (but generally better) world of "lualatex" and the
...
needs are simpler, then plain \MP\ is more than capable, and if you are already
using it for other illustrations there is one less thing to learn.

\parshape=1 0pt 216pt
Here is an example to illustrate some general techniques. 
\vadjust{\moveright 200pt\vbox to 0pt{\kern -28pt\includegraphics{xypic-example}\vss}}
It may be familiar to users
of "xypic". The code used to generate this picture
is shown on the right.
\vadjust{\moveright5.5in\vbox to 0pt{\kern-100pt
\begin{smallcode}
picture U, XY, X, Y, Z;
z1 = -z2 = (-61, 42);
...
\end{smallcode}
\vss}}
The approach taken here is to define the node labels first as pictures
placed as needed, and then to use \mpl{bbox} and \mpl{center} to connect them.
The "curved_connect" macro takes a string for the label, the two nodes
to be connected, and the initial direction for the curved path to connect them.

You can't have optional macro arguments in \MP\ but you can call macros from inside 
another macro, so here a second simpler macro "connect" is defined to do straight
connections.  To avoid repetition, this macro simply calls "curved_connect" with the 
required direction from $a$ to $b$.

\newpage
\section{A triangle of Schläfli polygons}

Apart from the curious patterns in the display, the main \MP\ point of interest

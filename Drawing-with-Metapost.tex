...
\textsc{Regular polygons} can be defined or drawn directly with a simple inline loop:
...
\end{code}
but you might prefer to make a macro:
\begin{code}
vardef polygon(expr n, r) = 
  for i = 0 upto n-1: r * dir (360 / n * i) -- endfor cycle 
enddef;
\end{code}
This produces a cyclic path that represents an $n$-sided polygon that fits in a circle of 
radius $r$ centred at the origin, and was used to create this polygonal version of
the previous segment chart $\longrightarrow$.

But you might want a polygon with a fixed side instead of a fixed radius. This needs
a little trigonometry, using the sine rule:
\begin{code}
vardef polygon_with_side(expr n, s) = 
    save a, b; numeric a, b; a * n = 360; a + 2b = 180; 
    for i = 0 upto n-1: 
        s * sind(b) / sind(a) * dir (a * i) -- 
    endfor cycle
enddef;
\end{code}

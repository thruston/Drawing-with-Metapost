...
of a path $p$ at any time $t$ along it: \mpl{direction t of p}.  This returns a pair
that represents the direction that the path is taking. In the language of Bezier
cubic curves, the pair is the value of the current control point minus the current
point on the path. So if you define a path like this:
\begin{code}
    path A;
    A = origin .. controls (72, 140) and (216, -160) .. (288, 0);
\end{code}

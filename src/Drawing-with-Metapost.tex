...
\textsc{The same techniques} that work for number lines also work for sequence
diagrams to show events instead of just numbers; by setting a unit you can keep
...
There are two extra macros required for this drawing.
...
but it works well for the up and down braces needed here.

\bigskip\noindent
The second it a bit trickier; it shows how to use a \mpl{text} parameter to allow
...
last annotation with the long red label shows.\par\vss}}
...
weeks) shown as $x$.  There are two series of labels: normal unit markings spaced as
required, plus annotations with pins and braces.
\mpexternal[linerange=25-42,xrightmargin=-36pt]{seq-timeline.mp} 

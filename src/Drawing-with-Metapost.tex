...
lets you write a \MP\ program, compile it, and include the results in your
...
or you can flick through until you see something that looks like it might
be useful and then see how it’s done.  A table of contents is at the end.
...
"colorbrewer") occasionally.  Nearly all of the examples here are supposed to be
self-contained, and any macros are defined locally so you can get to grips with
what’s going on.  \MP\ is a very subtle language, and it’s possible to do some very
clever and completely inscrutable things with it;  but here I have tried to be as
clear as possible in my examples.

\vbox to 0pt{
\vskip .75in
\noindent
\begingroup\footnotesize\blue{
Copyright \copyright\ 2026 by Toby Thurston. This material may be distributed only
...
\begin{itemize}
    \item \textbf{Assignment or equation}:  the \textit{equation} \mpl{a=3;} means
        “"a" is the same as "3" throughout the current scope”; the
        \textit{assignment} \mpl{a:=3;} means “update the value of "a" to the value
        "3" immediately”.  The difference becomes apparent when you try to update a
        variable in the same scope.  \mwpic{-64pt}{tiling-arch-hexagon}

        This difference also lets you write linear equations
        like \mpl{a=-b;}.  After this, as soon as you give  a value to "a", \MP\ immediately works
        out the value of "b".
...
\end{code}
    ...
    \item \textbf{Variable types}:
        ...
        If you don’t declare a variable, \MP\ assumes that it’s a \<numeric>.
        When you \textit{do} declare a variable --- \<numeric> or otherwise --- any value that it
       already had in the current scope is removed.

    \item \textbf{Implicit multiplication}: \MP\ inherits a rich set of rules about numerical
...
Skim.app to view PDF files.  I have the complete MacTeX distribution
...
name.  Usually, I open Skim.app to preview the
output.
\vadjust{\moveright5.4in\vbox to 0pt{\vss\hsize 4in\noindent
$$\includegraphics[width=4in]{workflow-plain}$$
Until recent (2024) versions of Mac~OS, it was possible to get Skim.app to view the
...
PDF file; remove each EPS file if successful.  If you are interested, have a look 
at "mppdf" in the "src" directory of this project.\par
}}
...
As noted above, a \<path> is a regular variable type in \MP.  So, while it is
perfectly valid syntax to write:
\begin{code}
    draw 5 up .. 30 dir 40 .. 60 dir -10 .. 90 right;    
\end{code}
to get this:
\[\begin{mplibcode}
draw 5 up .. 30 dir 40 .. 60 dir -10 .. 90 right;    
\end{mplibcode}\]
it is more useful, and perhaps more idiomatic, to produce the same result with:
\begin{code}
    path t; t = 5 up .. 30 dir 40 .. 60 dir -10 .. 90 right; 
    draw t;
\end{code}
In particular you can 
use the \<path> variable to make other paths, and 
you can apply transformations like \mpl{shifted} and \mpl{rotated} to draw copies or
make repeats.  For example, you could replace “\mpl{draw t;}” in the example above,
with:
\begin{code}
    for i = 1 upto 24: draw t rotated 15i; endfor
\end{code}
to get the pattern shown on the right.
\mnote{-200pt}{\centerline{\begin{mplibcode}
path t; t = 5 up .. 30 dir 40 .. 60 dir -10 .. 90 right; 
for i = 1 upto 24: draw t scaled 1.2 rotated 15i; endfor
\end{mplibcode}}}
...
\newpage
\subsection{Open paths and closed paths}

In \MP\ there are two sorts of paths: open and closed.
\mpic{12pt}{paths-triangles}
A closed path is also called a cycle, and is created with the \mpl{cycle} primitive like this:
\begin{code}
...
\smallskip
\noindent
\hey Note that you have explicitly put \mpl{cycle} to make a closed path.  If you wrote
...
path.\mpic{24pt}{paths-open-triangle}
...
\smallmpexternal[linerange=7-11]{paths-fixed-polygon.mp}
\noindent
Which you can use like this to produce a nest of polygons \rightarrowfill
\smallmpexternal[linerange=12-16]{paths-fixed-polygon.mp}
...
\smallmpexternal[linerange=6-22]{paths-polygon-chain.mp}
\vskip -42pt}}
...
\mpexternal[linerange=6-26]{paths-schlafli-polygons.mp}
...
with
\mpexternal[linerange=18-18]{paths-area-under-graph.mp}
\noindent
...
\end{itemize}}
\mpexternal[linerange=7-23]{paths-saturn.mp}
...
\mpexternal[linerange=7-14]{paths-overlaps-missing-filler.mp}
...
\mpexternal[linerange=9-46,xleftmargin=0pt]{clocks.mp}
\vss}}
...
$$\kw{beginfig}(1);{}\mathbin{\kw{draw}}\hbox{clock}(\id{hour},\id{minute}); \kw{endfig};$$
to give a sort of graphical time stamp.

\vfill

There is also a \mpl{round} function that rounds a number to the nearest integer.  It is
...
for x=1 upto 20:
  for y = 1 upto 9:
    pair w; w = (42y, -20x);
    if x = 1:
      label("$x" if y>1: & "^{" & decimal y & "}" fi & "$", w);
    elseif y * mlog(x) < mlog(infinity):
      numeric r; r = x ** y;
      if r = round(r):
        label(decimal r, w) withcolor 3/4 blue;
      else:
        label(TEX(decimal r) scaled 3/4, w) withcolor 3/4 red;
      fi
    fi
  endfor
endfor
...
label.lft("the default \mpl{scaled} number system.", lrcorner currentpicture shifted 24 up);
    label.lft("\small (It's even worse with the new number systems.)", lrcorner currentpicture shifted 12up);
currentpicture := currentpicture scaled 0.8;
\end{mplibcode}$$\vss}}
Note that the squares are all integers, and the powers of two are OK until you get 
to \mpl{2**9}, but that, with a couple of exceptions cubes, and higher powers are all slightly off.  Changing
...
\mpexternal[linerange=6-30]{pairs-complex-operators.mp}
\vss}}%
...
\smallmpexternal[linerange=11-44,xleftmargin=0pt]{pairs-complex-inverse-and-sqrt.mp}
\vss}}
...
\mpexternal[linerange=6-6]{pairs-complex-inverse-and-sqrt.mp}
...
\mpexternal[linerange=7-9]{pairs-complex-inverse-and-sqrt.mp}
...
\mpexternal[linerange=7-42,xleftmargin=0pt]{pairs-double-dragon.mp}
\vss}}
...
\vadjust{\moveright 396pt\vbox to 0pt{\kern-216pt\noindent
To use RGB hex strings, you'd need to write a function:
\begin{code}
...
\vskip 42pt\noindent
It is easy to define a few more common colours in RGB using the plain constants.
\begin{code}
...
Plain \MP\ defines just five colour constants: \mpl{red}, \mpl{green},
\mpl{blue}, \mpl{white}, and \mpl{black}.  These are quite useful with leading
...
easier to read.  Much more usefully, you can use the mediation syntax to get a
...
\mpexternal[linerange=6-14]{color-blend-toy.mp}
\vss}}
Here's a toy example:

\vbox to 0pt{\centerline{\includegraphics{color-blend-toy}}\vss}
...
\mpexternal[linerange=9-41]{color-blended-circles.mp}
}
\newpage
...
\mpexternal[linerange=5-31]{color-hsv-gamut.mp}}
...
\mpexternal[linerange=7-39, xleftmargin=0pt]{color-hsv-bathymetric.mp}

\newpage
\subsection{Grey scale}

\moveright5.4in\vbox to 0pt{\vskip6pt
\mpexternal[linerange=5-36,xleftmargin=0pt]{color-grey-escher.mp}
\vss}
\noindent
...
bottom to top.  If you set the loop step small enough, these multiple lines blend
smoothly to give an even colour.  And by using higher powers of the index variable,
an effective shadow can be drawn ‘bunched up’ into the top of each shape.  By
repeating the \<picture> variables alternately in a grid, you get an effective
texture.

\newpage
\subsection{Extended ranges of colour names}\label{sec:more-colours}

If you want to use a wider range of colour names you can load the "mpcolornames" 
package, which defines three sets of colours using the traditional names for DVIPS, SVG, and X11.
The package defines 68 \<cmykcolor> variables for DVIPS, 151 \<color> variables for
SVG, and over 300 \<color> variables for X11.
The
figure\mpic{-30pt}{color-dvips-names} at the right shows the charmingly
idiosyncratic DVIPS 
names. Notice that they all start with a capital letter, and each one needs a
suffix.  You can read the package documentation for full details.

\bigskip
\parshape 4 0pt 5in 0pt 4.8in 0pt 4.6in 0pt 4.5in
\noindent
If you are creating your \MP\ drawings with Lua\LaTeX\ and an up to date version
"luamplib", as recommended in §\ref{sec:sa-lua-flow}, then you can use colours
that are defined at the \LaTeX\ level.   This is particularly useful if you are 
creating a document with integrated \MP\ drawings (described in §\ref{sec:lua-integrated}).
Not only do you get access to the standard names, such as those shown here\rlap{\
$\rightarrow$}
but you also have access to names that you have defined
in the document.

\parshape 4 0pt 4.4in 0pt 4.4in 0pt 4.4in 0pt 4.4in
For example, the style file used with this current document defines two extra colours for 
syntax highlighting called \mpl{carrot} and \mpl{squash}, which could be used like
this:
\begin{code}
  fill fullcircle scaled 20 withcolor mplibcolor "squash";
  fill fullcircle scaled 12 withcolor mplibcolor "carrot";
\end{code}
to produce a roundel like this: $\vcenter{
\begin{mplibcode}
if scantokens(mpversion) > 2.02:
  fill fullcircle scaled 20 withcolor mplibcolor "squash";
  fill fullcircle scaled 12 withcolor mplibcolor "carrot";
else:
  fill fullcircle scaled 20 withcolor (0.793, 0.359, 0.05);
  fill fullcircle scaled 12 withcolor (0.934, 0.445, 0.082);
fi
\end{mplibcode}}$

\bigskip
Moreover, you can use any notation provided by the package you are\\
using  at the \LaTeX\ level; for example the "xcolor" blending notation, so\\
that you can write
\begin{code}
  fill fullcircle scaled 20 withcolor mplibcolor "green!70";
  fill fullcircle scaled 12 withcolor mplibcolor "blue!42!red";
\end{code}
to produce a different roundel: $\vcenter{
\begin{mplibcode}
if scantokens(mpversion) > 2.02:
  fill fullcircle scaled 20 withcolor mplibcolor "green!70";
  fill fullcircle scaled 12 withcolor mplibcolor "blue!42!red";
else:
  fill fullcircle scaled 20 withcolor 1/4[green,white];
  fill fullcircle scaled 12 withcolor 0.42[red,blue];
fi
\end{mplibcode}}$

\medskip
\noindent
\hey {\small The \mpl{mplibcolor} macro is implemented using the \mpl{withprescript}
mechanism, so it only works after \mpl{withcolor} as shown above. Note also that the 
colours are \<string> values here, not \<color> values.  Read the
"luamplib" documentation for details, or see below §\ref{sec:neo-access-to-tex}}.
...
\mpexternal[linerange=9-9]{color-brexit-map.mp}
\mpexternal[linerange=11-19]{color-brexit-map.mp}
\vss}

\noindent
The well-known Colorbrewer\mpic{-10pt}{color-brewer-mini-sample} website ("http://colorbrewer2.org") provides a useful set
...
\mpexternal[linerange=5-38,xrightmargin=-36pt]{random-dice.mp}
...
\mpexternal[linerange=5-31,xleftmargin=0pt]{random-walks-red-blue.mp}
\smallskip\noindent
...
\mpexternal[linerange=15-18,xleftmargin=0pt]{random-walks-normal.mp}
}}
...
\mpexternal[linerange=7-23,xleftmargin=0pt]{random-walks-normal-bridge.mp}
}}
...
\mpexternal[linerange=6-20,xleftmargin=0pt]{random-two-dimensional-brownian.mp}
...
\smallmpexternal[linerange=7-43,xleftmargin=0pt]{random-freehand-circumcircle.mp}
\vss}}
...
The Euler Math font complements the hand-drawn look.
...
\texternal[linerange=1-6,xleftmargin=0pt]{random-freehand-through.mp}
\vskip-\baselineskip
\mpexternal[linerange=7-35,xleftmargin=4pt, xrightmargin=-20pt]{random-freehand-through.mp}
\vskip-\baselineskip
\texternal[linerange=36-37,xleftmargin=0pt]{random-freehand-through.mp}
\vss}}

$$\includegraphics{random-freehand-through}$$

\vfill
\noindent\llap{\nb\ }If you prefer to use plain old "mpost", then the AMS Euler font
is still available as "eurm10", and it is encoded as a subset of the \TeX\
...
To make the example on the right work with "mpost" change \verb|"$\alpha$"| in the
label to \mpl{char 11 infont eurm10}, and so on. 
...
\hsize4in
\mpexternal[linerange=6-21,xleftmargin=0pt]{random-shapes.mp}
\vss}}
...
\mpexternal[linerange=8-29,xleftmargin=0pt]{random-explosions.mp}
\noindent
...
\smallmpexternal[linerange=6-37,xleftmargin=0pt]{random-qed.mp}
\vss}}
...
\mpexternal[linerange=6-12]{random-torn-straight-edge.mp}
...
\mpexternal[linerange=6-16]{random-torn-edge-circle.mp}
...
\vskip -12pt
\mpexternal[linerange=8-21,xleftmargin=0pt]{geometry-triangles-on-circle.mp}
\vss}}%
...
\mpexternal[linerange=5-7]{geometry-triangles-on-circle.mp}
...
\smallmpexternal[linerange=6-17]{geometry-trisection-simple.mp}
...
\mpexternal[linerange=8-18]{geometry-magic-square-14.mp}
\vss}
\noindent
...
\mpexternal[linerange=13-20]{geometry-intersection-all-three.mp}
\noindent
...
\mpexternal[linerange=27-32]{geometry-drawing-circles.mp}
\noindent
...
\mpexternal[linerange=11-18]{geometry-incircle.mp}
...
\mpexternal[linerange=11-18]{geometry-excircle.mp}
\noindent\hey
...
\smallmpexternal[linerange=11-16,xleftmargin=0pt, xrightmargin=-60pt]{geometry-circumcircle.mp}
\noindent
...
\smallmpexternal[linerange=38-47]{geometry-circumcircle.mp}
Given the triangular \<path> $T$, the macro was used like this:
$\longrightarrow$\vadjust{\moveright5.5in\vbox to 0pt{\vss
\smallmpexternal[linerange=49-51]{geometry-circumcircle.mp}
}}
...
\smallmpexternal[linerange=6-52,xleftmargin=0pt]{geometry-nine-point-circle.mp}
\vss}}
...
\mpexternal[linerange=11-13]{geometry-tangents-point-to-circle.mp}
...
\smallmpexternal[linerange=9-24]{geometry-tangents-two-circles-exterior.mp}
...
\mpexternal[linerange=7-10]{geometry-tangents-two-circles-interior.mp}
\noindent
...
\smallmpexternal[linerange=12-54]{geometry-tangents-two-circles-interior.mp}
\vskip -72pt
}}
...
them for you:
\mpexternal[linerange=6-11]{geometry-pole-and-polar.mp}
...
\moveright5.5in\vbox to 0pt{\vss\hsize 4in
\smallmpexternal[linerange=6-51]{geometry-radical-axis.mp}
\vskip -1in}
...
\textsc{Observant readers} will notice that the occasional angle marks in the
following examples are generally drawn using plain \MP\ commands rather than a
...
\medskip\noindent
Equipped with the simple macro shown on the left, you could do:
\begin{smallcode}
...
draw a--b--c--cycle;
    ...
    dotlabel.ulft("$c$", c);
\end{mplibcode}$$
Or, using the more generic macro, you could do:
\begin{smallcode}
draw angle_mark(a, c, b, 16) withcolor 2/3 red;
draw angle_mark2(c, b, a, 24) withcolor 1/2 blue;
\end{smallcode}
$$\begin{mplibcode}
pair a, b, c; a = 10 dir 10; b = 160 dir 20;
c - a = whatever * (b - a) rotated 90; ypart c = ypart b;
input geometry-angle-mark
draw unitsquare scaled 5 rotated angle (b-a) shifted a withcolor 3/4;
draw angle_mark(a, c, b, 16) withcolor 2/3 red;
draw angle_mark2(c, b, a, 24) withcolor 1/2 blue;
draw a--b--c--cycle;
...
idea here is that $a$, $b$, and $c$ are \<pair> variables and $r$ is a \<numeric>
representing the desired radius.
\begin{code}
vardef angle_mark(expr a, b, c, r) =
    fullcircle scaled 2r rotated angle (a-b)  % (a-b) is a <pair>
    shifted b cutafter (b--c)                 % (b--c) is a <path> 
enddef;
\end{code}

\smallskip\noindent
Less is
usually more in coordinate geometry diagrams, but if you like multiple line marks, 
you can make the macro more generic with a \<suffix> parameter:
\mpexternal{geometry-angle-mark.mp}
...
\mpexternal[xleftmargin=0pt,linerange=6-37]{geometry-examples-desargues.mp}
\moveright 340pt \vbox to 0pt{\vss
\mpexternal[xleftmargin=0pt,xrightmargin=-80pt,linerange=38-53]{geometry-examples-desargues.mp}
\vskip -42pt}

\newpage
\vpic{-36pt}{geometry-examples-trisections}
\vbox to 0pt{
    \mpexternal[xleftmargin=0pt,xrightmargin=-80pt,linerange=6-40]{geometry-examples-trisections.mp}
\vss}
\newpage
\kern-3\baselineskip
\vpic{0.25in}{geometry-examples-napoleon}
\vbox to 0pt{
    \mpexternal[xleftmargin=0pt,linerange=6-37]{geometry-examples-napoleon.mp}
\vss}
\newpage
\null
\kern-3\baselineskip
\vpic{1in}{geometry-examples-wallace-simson}
\vbox to 0pt{
    \mpexternal[xleftmargin=0pt,linerange=7-45]{geometry-examples-wallace-simson.mp}
\vss}
\newpage
\kern-3\baselineskip
\vpic{1in}{geometry-examples-projections}
\vbox to 0pt{
    \mpexternal[xleftmargin=0pt,linerange=6-41]{geometry-examples-projections.mp}
\vss}

\newpage
\moveright5.25in\vbox to 0pt{\hsize4.25in\noindent
\smallmpexternal[linerange=7-14,xleftmargin=0pt]{geometry-arbelos.mp}
...
\par\endgroup
\vss}
\smallmpexternal[linerange=15-52,xleftmargin=0pt,xrightmargin=-72pt]{geometry-arbelos.mp}
...
\mpexternal[linerange=1-37]{trigonometry-functions.mp}
\vss}}
...
precision.\rlap{\quad$\longrightarrow$}
...
in double quotes to make a string token; refer to a \<string> variable; or do
one of these things.
\begin{itemize}
    \item Concatenate two other strings with the splice operator: \mpl{&}.
...
\vss}}
\mpexternal[linerange=6-23,xrightmargin=-10pt]{trad-utf8.mp}
\noindent
...
\vss}}
\mpexternal[linerange=26-30]{trad-utf8.mp}
...
\mpexternal[linerange=5-18]{trad-greek-default-encoding.mp}
\centerline{\includegraphics{trad-greek-default-encoding}}

\mpexternal[linerange=6-8]{trad-greek-gfs-encoding.mp}
...
Modern type face.  The next few sections deal with some of the thing you might want
to do.
...
\mpexternal[linerange=9-10]{trad-callout.mp}
\noindent
...
\mpexternal[linerange=11-15]{trad-callout.mp}
\noindent
...
\smallmpexternal[linerange=1-14]{braces.mp}
\smallmpexternal[linerange=13-15]{trad-braces.mp}
\noindent
...
\textsc{If you are a maven of programming language syntax} you may have noticed that
\mpl{btex .. etex} fits into the type system that \MP\ inherits from \MF\ as a
\<picture> and not as a \<string>.  Effectively, \mpl{btex .. etex} act as a
special pair of quotation marks that create a \<picture>; however the contained text
is used
verbatim, so that the whole construction is a syntactical atom.  This means that you
\textbf{cannot} write this sort of thing:
\begin{code}
for i=0 upto 2: % this won't work
...
\end{code}
This works because the \mpl{TEX} macro is expecting a \<string> argument so the normal string
concatenation rules are applied before the macro is called.  The macro then does
this (where \mpl{s} is the argument name):
\begin{code}
write "btex "&s&" etex" to "mptextmp.mp";
write EOF to "mptextmp.mp";
scantokens "input mptextmp"
\end{code}
so that \MP\ gets the correct contents to pass to \TeX. 

The only trouble with this is that it makes \MP\ open a file,
write to it, close it, and then read it in again for each label one at a time; and this
means that it can be very slow.
The example on the right shows how to speed things up, by using the same file for
all the labels and only writing it once.

\moveright5.5in\vbox to 0pt{\vss\hsize4in\noindent
\mpexternal[linerange=7-16]{trad-dynamic-labels.mp}
$$
...
... point i of c shifted -center c
                 ...
                 shifted center c ...
\end{code}
The \mpl{write} command is a \MP\ primitive, and \mpl{EOF} is defined in
"plain.mp".
\vskip -48pt
}
...
\mpexternal[linerange=6-10]{trad-verbatim-with-infont.mp}
\noindent
...
\end{texcode}
\vskip-\baselineskip
\smallmpexternal[linerange=7-25]{neo-labels.mp}
\vskip-\baselineskip
\begin{texcode}[xleftmargin=0pt]
\end{mplibcode}
\end{document}
...
\end{texcode}
\vskip-\baselineskip
\smallmpexternal[linerange=8-26]{neo-labels-tte.mp}
\vskip-\baselineskip
\begin{texcode}[xleftmargin=0pt]
\end{mplibcode}
\end{document}
...
\vskip 58pt
\centerline{\red{$\star\quad\vcenter{\halign{\hss #\hss\cr
All the examples in the rest of this section\cr 
assume that you have set \texttt{\textbackslash mplibtextextlabel}\cr}}\quad\star$}}
\vss}}
you get this output:
...
\smallmpexternal[linerange=8-8]{neo-unicode.mp}
\noindent
or even whole paragraphs that use Unicode:
\smallmpexternal[linerange=9-14]{neo-unicode.mp}
...
\mpexternal[linerange=7-9]{neo-multi-line-labels.mp}
\noindent
...
\mpexternal[linerange=10-13]{neo-multi-line-labels.mp}
\noindent
...
\mpexternal[linerange=14-17]{neo-multi-line-labels.mp}
...
\mpexternal[linerange=8-11]{neo-simple-number-line.mp}
\noindent
...
\mpexternal[linerange=7-12]{neo-simple-circle-labels.mp}
...
\newpage
\subsection{Exchanging values between \TeX\ and \MP}
\label{sec:neo-access-to-tex}

Recent (2025) versions of "luamplib" provide operators and macros that 
give you access to colours and variables defined at the \LaTeX\ level.  

\subsubsection{Access to \LaTeX\ colours}

The \mpl{mplibcolor} macro mentioned in §\ref{sec:more-colours} is implemented using
the \mpl{withprescript} mechanism that allows you to insert PostScript code
and other specials into parts of a drawing; so it only works if you use it
immediately after \mpl{withcolor}.  If you want to do anything else with a \LaTeX\
colour, you need to use one of these:
\begin{itemize}
    \item \mpl{mplibtexcolor} takes a \<string> and returns a value that may be \<numeric>, 
        \<color>, or \<cmykcolor>, depending on how the colour is defined in \TeX.

    \item \mpl{mplibrgbtexcolor} takes a \<string> and returns a \<color>, 
        converting the \LaTeX\ colour to RGB if needed.
\end{itemize}
In both cases the \<string> argument should be a valid \LaTeX\ colour expression.

\subsubsection{Access to \LaTeX\ variables}

Reading from \TeX\ variables is limited to \<dimen> values, but this is usually
what is useful, for example you might use "\textwidth", or even
"\marginparwidth" to make an adaptable drawing.  The operator is:
\begin{itemize}
    \item \mpl{mplibdimen} \<string> $\longrightarrow$ \<numeric>
\end{itemize}
The \<string> must be a valid \LaTeX\ control sequence name that refers to a
\<dimen> value.  The name should include the leading backslash.  The result is
returned as a \<numeric> value converted to PostScript points, so if your text height
is set to 6~inches, \mpl{mplibdimen "\\textheight"}
will return 432 (approximately).

\smallskip
\noindent
Writing back to \TeX\ is more general, and uses the \mpl{VerbatimTeX} macro provided
by "luamplib".  You can use it like this:
\begin{code}
    numeric r; r = 42; % more likely r is a measured length...
    VerbatimTeX("\gdef\mpr{" & decimal r & "}");
\end{code}
After "luamplib" has processed this, the control sequence "\mpr" would expand to
\texttt{"42"}.
...
\end{texcode}
\vskip -\baselineskip
\smallmpexternal[linerange=8-24,xleftmargin=0pt,xrightmargin=-10pt]{neo-marked-up-photo.mp}
\small
...
\smallmpexternal[linerange=3-16,xleftmargin=0pt,xrightmargin=-36pt]{neo-reference-grid.mp}
...
\smallmpexternal[linerange=6-43,xleftmargin=0pt,xrightmargin=-44pt]{new-fangled-trilobe.mp}
...
\medskip
\mpexternal[linerange=6-27,xleftmargin=0pt]{pics-shady-circles.mp}
\vskip -126pt}}
...
-1.5\baselineskip
\smallmpexternal[linerange=8-29]{pics-draw-picture.mp}
...
\smallmpexternal[linerange=28-48]{pics-fake-transparency.mp}
\vss}}%
...
\smallmpexternal[linerange=8-11, xleftmargin=0pt]{pics-graph-paper-example.mp}
\noindent
...
Note that this macro is designed to be used a temporary input added at the
...
\smallmpexternal[xleftmargin=0pt,linerange=9-10]{pics-border-rope.mp}
\vss}}
...
\smallmpexternal[linerange=6-11]{pics-youth.mp}
\noindent
...
\smallmpexternal[linerange=18-64,xrightmargin=-40pt]{picture_frame.mp}
\vss}}

\noindent This macro also needs some colours:
\smallmpexternal[linerange=4-7]{picture_frame.mp}
\noindent a picture of a small silvery-gold ball:
\smallmpexternal[linerange=9-14]{picture_frame.mp}
\noindent and an internal variable that defines the width of the frame:
\smallmpexternal[linerange=16-16]{picture_frame.mp}
...
\smallmpexternal[linerange=8-32,xleftmargin=0pt]{pens-besides.mp}
...
\smallmpexternal[linerange=21-21,xleftmargin=0pt]{pens-strokes.mp}
\noindent
...
ends.
\smallmpexternal[linerange=26-26,xleftmargin=0pt]{pens-strokes.mp}
\noindent
The “railway line” uses a combination of three drawing operations:
\smallmpexternal[linerange=31-33,xleftmargin=0pt,xrightmargin=-30pt]{pens-strokes.mp}

\smallskip\noindent
The “plainer railway line” was done like this:
\smallmpexternal[linerange=40-44,xleftmargin=0pt]{pens-strokes.mp}
\noindent
...
\smallmpexternal[linerange=49-52,xleftmargin=0pt]{pens-strokes.mp}
\noindent
...
\smallmpexternal[linerange=6-21,xrightmargin=-42pt]{pens-greek-meander.mp}
...
(note that you need to use the assignment operator \mpl{:=} in order to update them).
...
\newpage
\section{Number lines and sequence diagrams}\label{timelines}

\textsc{Number lines} have a variety of uses in teaching or just for presenting a
sequence of events in a process.  The key to drawing them effectively in \MP\ is to
use a consistent unit size.  Here is a simple line showing numbers from $-4$ upto
$6$.
$$\includegraphics{seq-simple-line}$$
which was produced with a unit $u$ to scale the axis and to position each label.
\mpexternal{seq-simple-line-common.mp}
\noindent
{\small\hey But notice that even with this very simple starting point some care
is needed.  You might like to experiment with removing the "\strut" from the labels,
and you might like to think of a way to get the negative numbers to line up better
with the marks.}

\moveright5.5in\vbox to 0pt{\vskip -3.4in\hsize 4.2in
$$\includegraphics{seq-fractions}$$
\noindent 
Each of the fraction lines above was created with this macro:
\smallmpexternal[linerange=8-20]{seq-fractions.mp}
and with the "gcd" macro from §\ref{sec:gcd}. Notice that this time, 
the tick marks are placed at fractions of the desired width, instead of multiples
of a unit.
\vss}

\noindent
You can use this as the starting point for more ambitious number lines:
$$\includegraphics{seq-simple-line-with-jumps}$$
\mpexternal[linerange=8-16]{seq-simple-line-with-jumps.mp} 

\newpage
\subsection{Time lines}

\textsc{The same techniques} that work for number lines also work for 
diagrams that show events in time instead of just numbers; by setting a unit you can keep
control of the relative spacing of events.  But you may need finer control
over your labels.
\vadjust{\moveright5.5in\vbox to 0pt{\vskip 44pt\hsize4.5in\noindent 
There are two extra macros used in this drawing for the labels.

\smallskip\noindent
The first makes a brace \<path>, given a start, a middle, and an end point.  
\mpexternal[linerange=18-23]{seq-timeline.mp}
This macro is not very general because it assumes that $a$ and $b$ are horizontal,
but it works well for the up and down braces needed here (see also
§\ref{sec:braces} for a different approach).
It is tempting to make a macro to combine the pin and/or the brace with the label to
avoid repetition, but keeping them separate allows more flexible colouring as the
last annotation with the long red label shows.

\bigskip\noindent
The second macro is a bit trickier; it shows how to use a \mpl{text} parameter to allow
for a flexible number of arguments, and how to use \mpl{scantokens} to turn a string
into a number, even when it includes a variable name.
\mpexternal[linerange=9-16]{seq-timeline.mp}
\vss}}
$$\includegraphics[width=\textwidth]{seq-timeline}$$
In this diagram, the unit size was one month, with a variable period (of about three
weeks) shown as $x$.  There are two series of labels: 
annotations with pins and braces; 
plus regular unit markings spaced as
required.
\mpexternal[linerange=25-43,xrightmargin=-36pt]{seq-timeline.mp} 

\newpage
\subsection{Vertical time lines}

\vpic{-24pt}{seq-covid-in-wales}

\mpexternal[linerange=7-40]{seq-covid-in-wales.mp}

\newpage
\subsection{Time lines with calendar dates}

\textsc{If you are illustrating} a time line of real events, you may find
it easier to work with calendar dates and use a macro to reduce them 
to a scalar number.  The \mpl{base} macro shown in §\ref{sec:basedate} does this 
for you.\mpic{-63pt}{seq-covid-in-wales}  This macro takes three
whole number \<numeric> arguments ("year", "month", "day") and returns 
a \<numeric> serial number.  The macro is designed to work with the default scaled number system, 
so the serial number is returned as a real number, scaled so that one day equals
exactly $1/1024$. So \mpl{base(1,1,1)} returns 0, and \mpl{base(2001,1,1)} returns
$713.36426$.

For the diagram on the right, it was convenient to turn each date 
into a \<pair> scaled to move down the vertical axis:
\smallmpexternal[linerange=11-14]{seq-covid-in-wales.mp} 
Subtracting the start date means that the scale starts at zero, and
scaling by $-512$ means that each year takes about 6.45\unit{cm}
so the two-and-a-half year time span fits neatly on the page.
This "date" macro was used in another macro to mark events:
\smallmpexternal[linerange=20-26]{seq-covid-in-wales.mp} 
and so the first events were created like this:
\smallmpexternal[linerange=55-56]{seq-covid-in-wales.mp} 
Notice that the text for the label is passed as a string, and processed
by the "TEX()" macro.  And also that it is convenient to pass an offset 
so that you can separate consecutive labels when the dates are close togther.
A similar macro was used to mark the periods with braces. See the source code for 
details.

\vfill

{\small\noindent\hey This diagram was adapted from one originally created by the
BBC.
You might like to think about 
why the vertical format works, and how you could re-design it if you needed
a horizontal scale.}
...
\smallmpexternal[linerange=6-10]{func-plain-axes.mp}
\noindent
...
\vskip -12pt
\vbox to 0pt{
\smallmpexternal[linerange=7-21]{func-numbered-axes.mp}
\vss}
...
just to specify two points and use \mpl{draw}; often it is handy to make your line
...
\smallmpexternal[linerange=6-49]{func-linear-graph.mp}
\vss}}
...
The lines are drawn trimmed to a suitable \<path> (called \id{invisible\_boundary}).
\smallmpexternal[linerange=6-12]{func-angles-lines.mp}
\vskip-12pt\continued
\moveright 384pt\vbox to 0pt{\hsize 4in\vss
\smallmpexternal[linerange=14-51]{func-angles-lines.mp}
\vskip -12pt}
...
\mpexternal[linerange=15-23]{func-reflection.mp}
...
\smallmpexternal[linerange=6-40,xleftmargin=0pt]{func-exponential.mp}
...
\mpexternal[linerange=15-50]{func-sines.mp}
\vss}}
...
\smallmpexternal[linerange=6-13]{func-sines.mp}
...
\mpexternal[linerange=22-56, xrightmargin=-20pt]{func-addition-of-sines.mp}
\vss}}
...
\newpage
\subsection*{Manipulating functions arithmetically}

\textsc{This third example} with trigonometric functions shows a second way to add
two functions.  This approach is more mathematical, and perhaps more natural.
\vadjust{\moveright5.25in\vbox to 0pt{\vskip-28pt
\mpexternal[linerange=7-37, xrightmargin=-20pt]{func-damped-cosine-oscillation.mp}
\vss}}
$$\includegraphics[width=\textwidth]{func-damped-cosine-oscillation}$$
\begin{itemize}
    \item Notice how the inline for-loops help you organize the code visually.

    \item In this example you know that $e^0=\cos 0 = 1$, so the first point
        of each curve is going to be $(0,1)$ before scaling — which you can write as
        \mpl{up}.

    \item Recall also that \mpl{mexp(x)} is defined as $e^{x/256}$ so with
        \mpl{mexp(-4t)}, 
        you have $\gamma/2 = 4/256 = 1/64$ — this controls steepness of the
        exponential curve.  

    \item 180 points is plenty to get nice smooth curves, and \mpl{cosd(8t)} ensures
        that there are four full cycles in the graph.
...
\smallmpexternal[xleftmargin=0pt, linerange=8-42]{func-epi-v-pie.mp}
\vskip-12pt\continued
\moveright384pt\vbox to 0pt{\vss\hsize 4in\null\bigskip
\smallmpexternal[linerange=44-57]{func-epi-v-pie.mp}
}
...
\vskip-12pt
\smallmpexternal[linerange=6-40,xleftmargin=0pt, xrightmargin=-300pt]{func-stress.mp}
...
\mpexternal[linerange=7-16]{func-pulse.mp}
...
\smallmpexternal[linerange=6-21]{func-lemniscate.mp}
\noindent
...
\smallmpexternal[linerange=12-14]{func-parametric.mp}
\noindent
...
\mpexternal[linerange=6-23]{func-maurer-rose.mp}
...
\vss}}
\mpexternal[linerange=6-26,xleftmargin=0pt,xrightmargin=-36pt]{curves-parabola.mp}
...
to define the focus:
\mpexternal[linerange=17-31]{curves-parabola-directrix.mp}
...
\mpexternal[linerange=10-11]{curves-ellipse.mp}
\noindent
Then the lengths of the semi-major axes, $a$ and $b$, were extracted like this:
\mpexternal[linerange=13-15]{curves-ellipse.mp}
\noindent
...
\mpexternal[linerange=17-17]{curves-ellipse.mp}
\noindent
The focus points can then be found like this:
\mpexternal[linerange=19-21]{curves-ellipse.mp}
\noindent
...
\mpexternal[linerange=39-46]{curves-ellipse.mp}
}
...
\mpexternal[linerange=7-44,xleftmargin=0pt,xrightmargin=-36pt]{curves-ellipse-tangents.mp}
\vskip -36pt}}
...
\smallmpexternal[linerange=13-16]{curves-cardioid-construction.mp}
\noindent
...
\smallmpexternal[linerange=12-15]{curves-limacon-simple.mp}
\noindent
...
\smallmpexternal[linerange=8-12,xleftmargin=0pt]{curves-astroid.mp}
\noindent
...
\mpexternal[linerange=7-14]{curves-astroid-family.mp}
}
...
\smallmpexternal[linerange=5-46,xleftmargin=-10pt]{curves-cycloids.mp}
\vskip -64pt}
...
\smallmpexternal[linerange=6-42]{curves-brachisto.mp}
\vss}}
...
\mpexternal[linerange=7-9,xleftmargin=0pt]{curves-spiral-equiangular.mp}
\noindent
...
\mpexternal[linerange=6-33,xleftmargin=0pt]{curves-spiral-gnomon-sq.mp}
...
\mpexternal[linerange=6-36,xleftmargin=0pt, xrightmargin=-80pt]{curves-spiral-gnomon-trig.mp}
\moveright 396pt \vbox to 0pt{\vss\hsize 4in\small
...
\mpexternal[linerange=5-29,xleftmargin=0pt]{eggs-moss.mp}
\noindent
\newpage
...
\mpexternal[linerange=8-17]{eggs-thom.mp}
\noindent
...
\mpexternal[linerange=9-17,xleftmargin=0pt]{eggs-four-point.mp}
...
\smallmpexternal[linerange=6-37,xleftmargin=0pt]{eggs-five-better.mp}
...
\mpexternal[linerange=6-8]{eggs-super.mp}
...
\mpexternal[linerange=9-13]{eggs-perfect.mp}
\noindent
...
\smallmpexternal[linerange=6-20,xleftmargin=0pt]{eggs-shaded.mp}
\noindent
...
\smallmpexternal[linerange=6-20,xleftmargin=0pt,xrightmargin=-24pt]{eggs-fried.mp}
...
\smalltexternal[linerange=1-7,xleftmargin=0pt]{tufte-mpg.mp}
\vskip-12pt
\smallmpexternal[linerange=8-36,xleftmargin=0pt]{tufte-mpg.mp}
\vskip-12pt
\smalltexternal[linerange=37-38,xleftmargin=0pt]{tufte-mpg.mp}

\newpage
\subsection{Time series with minimal annotation}
...
\end{itemize}}
\smallmpexternal[linerange=10-44,xleftmargin=0pt,xrightmargin=-30pt]{tufte-budget.mp}

\newpage
\subsection{Time series with more complex dates}\label{sec:basedate}

If the dates in your time series are more granular, then you need a
...
\end{code}
for suitable values of $u$ and $v$. \hfill\ding{43}\ \textit{See the source file for
details}\\
...
\smallmpexternal[linerange=11-48,xleftmargin=0pt]{tufte-snow.mp}
\noindent\hey
...
\mpexternal[linerange=6-42,xleftmargin=0pt,xrightmargin=-20pt]{cd-xypic-example.mp}
\noindent
...
\mpexternal[linerange=26-31]{cd-tikzcd-example.mp}
...
§\ref{sec:gcd}.
\smallmpexternal[linerange=1-1]{rec-gcd-comparison.mp}
\noindent
The reason this works is that we know (mathematically) that $0 \le a \bmod b < b$,
and therefore that the second argument to the recursive call must always be smaller
than the first and eventually it must get to $0$ when the answer will be the first
argument.  This is the basic
...
\smallmpexternal[linerange=15-20]{rec-gcd-comparison.mp}
\noindent
...
\mpexternal[linerange=7-18]{rec-koch-steps.mp}
\noindent
...
\mpexternal[linerange=6-15,xleftmargin=0pt,xrightmargin=-72pt]{rec-sierpinski-triangle.mp}
\noindent
...
\mpexternal[linerange=16-19,xleftmargin=0pt,xrightmargin=-72pt]{rec-sierpinski-triangle.mp}
\noindent
...
\smallmpexternal[linerange=6-15]{rec-dragon.mp}
\noindent
...
corners.
\smallmpexternal[linerange=17-23]{rec-dragon.mp}
\noindent
...
\smallmpexternal[linerange=6-13,xleftmargin=0pt]{ifs-heigh.mp}
\noindent
...
\smallmpexternal[linerange=6-13,xleftmargin=0pt]{ifs-heigh-open.mp}
\noindent
...
level parameter:
\smallmpexternal[linerange=12-20]{rec-dragon-golden.mp}
\vss}
...
\smallmpexternal[linerange=18-38]{rec-flowsnake.mp}
\vskip -48pt}}
...
\mpexternal[linerange=7-16]{rec-simple-tree.mp}
\noindent
...
\mpexternal[linerange=18-26]{rec-simple-tree.mp}
\noindent
...
\smallmpexternal[linerange=6-31,xleftmargin=0pt]{rec-general-tree.mp}
\noindent
\vbox to 0 pt{\begin{itemize}
...
\smallmpexternal[linerange=17-17,xleftmargin=0pt]{rec-general-tree-deviate.mp}
...
\mpexternal[linerange=8-17,xleftmargin=0pt,xrightmargin=-150pt]{rec-bush.mp}
...
\mpexternal[linerange=8-12,xleftmargin=0pt,xrightmargin=-12pt]{tiling-simple.mp}
\noindent
...
\mpexternal[linerange=16-28,xleftmargin=0pt,xrightmargin=-12pt]{tiling-simple.mp}
\noindent
...
\mpexternal[linerange=11-15,xleftmargin=0pt]{tiling-hex-trig.mp}
\noindent
In the first row, $P$ was set to a simple polygon path:
\mpexternal[linerange=28-29,xleftmargin=0pt]{tiling-hex-trig.mp}
\noindent
The vectors $u$ (in red) and $v$ (in blue) were defined as (for both tilings):
\mpexternal[linerange=31-32,xleftmargin=0pt]{tiling-hex-trig.mp}
\noindent
...
\newpage
\subsection{Tiling with other shapes on regular grids}

\textsc{In some drawings} you may want a grid that illustrates an arrangements of cells or other
discrete objects, while these are not strictly tilings in the mathematical sense,
you can use the techniques introduced on the previous page to make a grid of shapes
that do not quite touch.
...
\<picture> for it.
\mpexternal[linerange=5-31]{tiling-arch-4-8-8.mp}
\noindent
...
\mpexternal[linerange=7-36,xleftmargin=0pt]{tiling-arch-3-4-3-4.mp}
...
\mpexternal[linerange=23-37,xleftmargin=0pt]{tiling-arch-snub-hex-parts.mp}
\noindent
...
\smallmpexternal[linerange=6-30]{tiling-arch-3-4-6-4.mp}
\noindent
...
\mpexternal[linerange=7-26,xleftmargin=0pt]{tiling-pp48a.mp}
\noindent
...
\mpexternal[linerange=7-28]{tiling-subs-triangle.mp}
...
\smallmpexternal[linerange=1-15]{tiling-pinwheel-macro.mp}

\vskip -\baselineskip

\smallmpexternal[linerange=7-14]{tiling-subs-pinwheel.mp}
...
\smallmpexternal[linerange=10-48, xleftmargin=0pt, xrightmargin=-30pt]{tiling-ammann-procedures.mp}
\vss}}
...
\smallmpexternal[linerange=10-14, xleftmargin=0pt, xrightmargin=-30pt]{tiling-ammann-procedures.mp}
\smallmpexternal[linerange=50-81, xleftmargin=0pt, xrightmargin=-30pt]{tiling-ammann-procedures.mp}
\vss}}
...
-36pt\smallmpexternal[linerange=7-46, xleftmargin=0pt]{tiling-penrose-tatham-p2-kite-dart.mp}\vss}}
...
\smallmpexternal[linerange=7-46, xleftmargin=0pt]{tiling-penrose-tatham-p3-rhombs.mp}
\vss}}
